...
**Acceso a recursos:** El script completo con los ejemplos desarrollados y la base de datos sobre esporofitos están disponibles para su consulta y descarga en el siguiente repositorio: `[ADJUNTAR ACA EL ENLACE AL REPOSITORIO]`
...
names(datos_esporofitos) <- c("frasco", "altura", "cantidad")
...
**El modelo ajustado es:** `altura=564.86−24.59×cantidad`

**Donde:**

1.  **Intercepto (**β0=564.86**):** El intercepto representa la altura estimada de los esporofitos cuando la cantidad de esporofitos germinados es cero. Matemáticamente, si no hubiera esporofitos germinados en un frasco, la altura estimada sería de 564.86 mm. Sin embargo, en el contexto biológico, este valor puede carecer de sentido práctico, ya que no es realista tener altura sin esporofitos germinados, pero es necesario para la ecuación del modelo (Kutner et al., 2005).

2.  **Pendiente (**β1=−24.59**):** La pendiente indica que, por cada esporofito germinado adicional en el frasco, la altura promedio de los esporofitos disminuye en 24.59 mm. Este valor negativo sugiere una relación inversa entre la cantidad de esporofitos germinados y la altura de los esporofitos: a mayor cantidad de esporofitos germinados, menor es la altura promedio de los mismos.
...
5.  **Error estándar de los residuos:** El error estándar de los residuos es 9.29, lo que indica que, en promedio, las predicciones del modelo difieren de los valores observados en aproximadamente 9.29 mm.
...
nuevos_datos <- data.frame(cantidad = c(10, 15, 20))
...
En este ejemplo, se predice la altura de los esporofitos para frascos con 10, 15 y 20 esporofitos germinados. El resultado será un vector con los valores estimados de altura para cada caso.
...
En primer lugar, se identificó una relación lineal negativa y significativa entre la cantidad de esporofitos germinados y la altura de los esporofitos. Específicamente, el modelo estimó que por cada esporofito germinado adicional, la altura promedio de los esporofitos disminuye en aproximadamente 24.59 mm. Este hallazgo sugiere que, bajo las condiciones experimentales descritas, un mayor número de esporofitos germinados en un frasco se asocia con un menor crecimiento en altura de los mismos, lo que podría estar relacionado con la competencia por recursos en el medio de cultivo (Rosales Castillo, 2005).
...
En síntesis, el análisis realizado demuestra que la cantidad de esporofitos germinados es un factor determinante en la altura de los esporofitos en el contexto experimental evaluado. Estos resultados pueden ser de utilidad para optimizar las condiciones de micropropagación y orientar futuras investigaciones sobre el crecimiento y desarrollo de esporofitos en medios de cultivo in vitro.
